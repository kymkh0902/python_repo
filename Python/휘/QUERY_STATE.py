...
def find_das2(grade, date1, date2):
    return("select unique_lot_no, prod_cd, normal_qty + normal_corr_qty prod_qty from tb_iem120 where prod_wc_cd like '%CC%' and prod_cd like '%{}%' and prod_date between '{}' and '{}'".format(grade, date1, date2))

def find_today_lot(date):
    return("select unique_lot_no, normal_qty + normal_corr_qty as prod_qty from tb_iem120 where prod_wc_cd like '%CC%' and prod_date = '{}'".format(date))
...
#Lot 찾기
def find_lot(lot):
    return("select F_ONE_LOT_TRACE_T(ar_warhs_create_no, '_E%', '%%') as 연신Lot, F_ONE_LOT_TRACE_T(ar_warhs_create_no, '_C%', '%%') as 코팅Lot from tb_iem120 where unique_lot_no = '"+lot+"'")
    
#Grade 별 생산 찾기
def find_grade(date, grade):
    return("select unique_lot_no as Lot, normal_qty + normal_corr_qty as 생산량, prod_cd as 코드 from tb_iem120 where prod_wc_cd like '%CC%' and prod_date ="+"'"+date+"'"+"and prod_cd like"+"'%"+grade+"%'")
...
def inspection_hist(date1, date2):
    return(
    '''       
       SELECT *
    FROM
...
def yt_lot_find():
	...
	F_FIND_CC_LOT(a.global_create_no) 코팅, normal_qty + normal_corr_qty 양품수, MAX(disproduct_qty) 불량수, unique_lot_no 검사, d.prod_cd 제품명, c.disp_factor_nm 불량명, prod_wc_cd 생산호기 FROM tb_iem401 b

	INNER JOIN tb_iem120 a ON a.ar_warhs_create_no = b.ar_warhs_create_no

	INNER JOIN (SELECT * FROM tb_iem930 WHERE plant_cd = 'G181' AND dong_cd ='G18101') c ON b.disp_factor_cd = c.disp_factor_cd

	INNER JOIN tb_iem903 d ON d.prod_cd = a.prod_cd

     WHERE prod_date BETWEEN '20170227' AND '20170228' AND prod_wc_cd LIKE '%FS%' and d.prod_nm like '%LH%' and a.del_flag = 'A' GROUP BY a.global_create_no, unique_lot_no, d.prod_cd, normal_qty + normal_corr_qty, c.disp_factor_cd, c.disp_factor_nm, prod_wc_cd

     ORDER BY unique_lot_no 
     '''
     )
