...
class output():
    ...
    def counting(data, size1, size2, value1, value2, include_lowest=True):
        ...
        """
        if include_lowest:
            defect_count = len(data[(data['size'] >= size1) & (data['size'] < size2) & (data['value'] >= value1) & (
            data['value'] < value2)])
        else:
            defect_count = len(data[(data['size'] > size1) & (data['size'] < size2) & (data['value'] > value1) & (
            data['value'] < value2)])
...
class preprocessing():
    ...
    def base(data, *args):
        ...
        *args : 데이터 불량 번호(int), 순차적으로 입력
        ...
        """
        if len(args) != 0:
            data = data[data['불량번호'].isin([i for i in args])]
...
def division(data, width, length, inch_x, inch_y, axis, pitch=None):
    ...
    data['cut_x'] = pd.cut(data['x'], bins=bins_x, labels=labels_x)
    data['cut_y'] = pd.cut(data['y'], bins=bins_y, labels=labels_y)
    chip_qty = len(labels_x) * len(labels_y)

    return data, chip_qty
